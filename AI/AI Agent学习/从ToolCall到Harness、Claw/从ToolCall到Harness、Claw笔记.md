...
根本原因还是上下文窗口是有限的
context engineer四大思想
- 写入（保存到上下文窗口之外）
- 选择（外部储存中“拉取”最相关的文档）
- 压缩（压缩上下文窗口的内容进行浓缩或者裁剪）
- 隔离（不同任务或者逻辑内容进行隔离）

| 症状   | 症状解释                                            | Context Engineer 解法               |
| ---- | ----------------------------------------------- | --------------------------------- |
| 遗忘漂移 | 当任务拉长到第 10 轮对话时，模型常常会忘了最开始的目标是什么                | Notepad / Todo (外部记忆)、分层记忆 & 工程约束 |
| 窗口超载 | Token 窗口是有限的，而且塞进去的垃圾信息越多，模型推理能力就越差             | 压缩机制 (Compaction)、动态选择工具          |
| 静态死板 | 传统的 Agent 往往靠一段长达几千字的 System Prompt，role 和能力被定死 | 动态 Prompt 组装、路径作用域规则              |
| 输出歧义 | 大模型随机生成性导致格式乱七八糟，后续基于输出的任务可能崩溃                  | 结构化输出 (Schema)                    |
举个例子：
【任务目标】：“在项目中新建一个获取天气数据的Python模块，编写对应的单元测试，并生成一份Markdown格式的API接口文档。”
![[Pasted image 20260723000002.png]]

|                             | Before: 传统工程                                                                                                             | After:上下文工程                                                                                                                          |
| --------------------------- | ------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------ |
| <center>动态拼接prompt</center> | 载入一段长达千字、永远不变的静态,你是一个全能的AI,System Prompt:"你是一个全能的AI助手,你会编程、写作、画画....."<br><br>后果: 模型漂移忘记Prompt,开始拉屎，写js或其他代码。            | 1. 自动加载.claw/settings.json,把"本系统只用Python3.10"作为Base Prompt注入。<br>2. Base(人设)+Domain(当前是个代码项目)拼装在一起。<br><br>优点: 动态加载能力和Base,不同阶段方便转换。 |
| <center>TODO</center>       | 大模型用自然语言写了一段:"好的,我打算第一步...第二步..."。这段话很快就被淹没在后续的聊天记录里。<br><br>后果：模型漂移忘记任务要求后续完不成所有任务，或者记错拉屎                               | 1. 强制按JSONSchema输出计划。<br>2. 将任务转换为持久化的[Todo任务板],<br>永远钉在每次对话的最顶部。<br><br>优点：不会忘记任务                                                   |
| <center>动态选择Tool</center>   | <p align="justify">模型脑子里装着所有100个工具。为了写个代码,它可能会误调用web_search或者send_email工具。<br><br>后果: 模型漂移,选择工具时候乱用</p><br><br>          | <p align="justify">监测到进入src/目录,立刻只保留read_file、write_file等3个代码工具,并附上代码规范规则。<br><br>优点: 精确限定Tool,节省上下文保证模型专注,简洁高效</p><br><br>          |
| <center>NotePad日志</center>  | <p align="justify">模型不断重试,每次工具执行后,几百行的ErrorStackTrace(报错日志)被原封不动地塞进历史消息里。<br><br>后果: 上下文爆炸性能下降或者窗口超载直接Error</p><br><br>  | <p align="justify">模型把错误猜想记入单独的Notepad(.md)。遇到异步报错,通过Catalog检索,此时才把几千字的异步调试手册塞给模型。<br><br>优点: Notepad记录关键信息。适时提供指导</p><br><br>       |
| 压缩机制                        | <p align="justify">循环了15次后,堆积如山的冗余日志导致Token爆满,或者模型注意力完全涣散,开始胡言乱语。任务直接失败中断。<br><br>后果: 上下文爆炸性能下降或者窗口超载直接Error</p><br><br> | <p align="justify">内存快满时,触发预警（设置hook）。将前14次失败的日志删除,用小模型总结为"重试14次失败,代理问题"。<br><br>优点:动态删除压缩,理论上无限续航</p><br><br>                       |
| 作用域切换                       | <p align="justify">模型还沉浸在写代码的状态里,写出的文档充满底层的代码细节,甚至用代码块来代替正常的文本描述。<br><br>后果:角色切换能力切换不来</p>                               | 进入docs/目录。抽走代码规则,换上rules/docs.md(要求清晰易读)。Todo面板自动勾选[x]代码部分。<br><br>优点: 动态切换角色,快速调整能力                                                 |
| 清空Memory                    | <p align="justify">告诉用户"做完了"。但是如果你再问它下一个任务,刚才debug产生的几十万垃圾Token依然在这个会话里污染脑力。<br><br>后果: 寿命短暂,可能一个Task也完成不了</p><br><br>   | <p align="justify">输出标准final_report。将最终成果写入长期数据库,然后将工作区记忆彻底清空,迎接下个任务。<br><br>优点: 既方便之后相同问题解决又为下一次任务清空腾出Context</p><br>               |
|                             |                                                                                                                          |                                                                                                                                      |
## 第六部分 Harness Engineer
>为Runtime保驾护航
完成基础能力搭建的模型，就像能力出众但完全不懂流程规范的实习生，或是能跑却未被驯服的野马，自身无法处理四类系统性风险：一是误输出删除等高危命令，无监管时会直接搞崩服务器；二是悄悄生成垃圾文件蚕食存储空间，不易被察觉；三是长周期任务遇断电、宕机会丢失进度，只能从头重跑，造成大量时间损耗；四是无成本意识，会无节制调用 API 刷爆额度。这些模型自身无法解决的问题，都需要上升到系统层面搭建兜底保障。

| 需求         | 传统工程解法                                              | Harness Engineer 实现                       |
| ---------- | --------------------------------------------------- | ----------------------------------------- |
| 安全可控（权限控制） | 权限控制 (Access Control)<br>拦截器 (Interceptor)          | 拦截操作<br>Gate (审批与门禁)<br>Enforce (机械约束)    |
| 高可靠（停电宕机）  | 容错性 (Fault Tolerance)<br>状态机持久化 (State Persistence) | Persist (检查点与断点续跑)                        |
| 环境隔离       | 沙箱化 (Sandboxing)<br><br>容器隔离 (Containerization)     | Isolate (沙箱与工作区)<br><br>Subagents (子代理隔离) |
| 可观测性（日志）   | 可观测性 (Observability - Logs/Metrics/Traces)          | Observe (链路追踪与反馈闭环)                       |

【高危任务背景】:"重构核心数据库的迁移脚本,在本地执行测试跑通,然后将代码合并,并部署到Staging(预发布)服务器。
![[Pasted image 20260723012628.png]]


| 阶段   | Before：传统工程                                                                              | After：驾驭工程                                                                                               |
| ---- | ---------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------- |
| 启动阶段 | 直接在你的宿主电脑当前目录下开干。它随意创建temp.sql,悄悄改了你的全局Python环境变量,甚至下载了带有冲突的底层依赖包。<br><br>后果:全局环境污染,目录杂乱 | Harness引擎拦截启动请求,自动分配一<br>个临时GitWorktree或Docker沙箱(Sandbox)。Agent只能在这个"透明隔离罩"里折腾。<br><br>优点:隔离环境,完成任务一尘不染。 |
| 编码阶段 |                                                                                          |                                                                                                          |
| 高危操作 |                                                                                          |                                                                                                          |
| 意外中断 |                                                                                          |                                                                                                          |
| 遇到报错 |                                                                                          |                                                                                                          |
...
[[Agent项目实践篇规划]]
![[Pasted image 20260728175800.png]]
