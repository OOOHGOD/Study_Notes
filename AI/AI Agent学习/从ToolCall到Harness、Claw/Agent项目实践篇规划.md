---
title: MokioClaw 项目篇规划
summary: 规划通过六个阶段从零实现 Code Agent 的视频课程、功能演进和技术选型。
tags:
  - AI-Agent
  - Code-Agent
  - MokioClaw
...
![[Pasted image 20260729182358.png]]
...
**Tool工具清单：**
> 工具获取的网站资源：
![[Pasted image 20260729184445.png]]
```
[学习 Claude Code | Claude Code源码学习专题](https://www.xuanyuancode.com/learn-claude-code)
```

| 工具名称          | 核心功能            | 使用必要性说明                    |
| ------------- | --------------- | -------------------------- |
| FileReadTool  | 读取工作区内文件内容      | Agent 修改文件前必须先读取原文，获取完整上下文 |
| FileWriteTool | 新建文件 / 完整覆写已有文件 | 用于从零生成全新代码、配置文件            |
| FileEditTool  | 精准局部替换文件指定片段    | 仅修改代码小块，无需全量重写文件，效率更高      |
| GrepTool      | 基于正则表达式检索文件内容   | 快速匹配、定位目标代码片段所在位置          |
| BashTool      | 执行 Shell / 终端命令 | 运行程序代码、安装第三方依赖、执行系统操作      |
...
> 每个工具闭包捕获 `state`（含 workspace 路径），所有文件操作限制在 workspace 内。`model.bind_tools(build_tools(state))` 把这些函数变成 LLM 可调用的 tool schema。[[闭包（Closure）是什么]]
### 🤖 Vibe Coding Prompt
