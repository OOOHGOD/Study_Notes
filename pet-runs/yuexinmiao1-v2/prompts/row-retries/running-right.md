Create Codex pet row `running-right` for `yuexinmiao1`: exactly 8 full-body frames in one horizontal strip on flat pure green #00FF00.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: Preserve the exact cheerful cartoon kitten identity, face, palette, proportions, outlines, and sticker-pack aesthetic from the supplied v1 spritesheet.. Preserve silhouette, face, palette, material, proportions, markings, and props.
